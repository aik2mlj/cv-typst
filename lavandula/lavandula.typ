...
#let section-element-advanced(title: "", info-top-right: "", info-top-left: "", icon: "", body) = {
  block(
    ...
    grid(
      ...
      column-gutter: 12pt,
      align: (col, row) => {
        if col == 0 and row == 0 { horizon + center } else if col == 0 and row == 1 { top + center } else if (
          col == 1 and row == 1
        ) { top + left } else if col == 1 and row == 0 { horizon + left } else if col == 2 { horizon + right }
      },
...
#let lavandula-theme(custom-colors: (:), body) = context {
  ...
  colors.update((
    ...
    highlight: new-base-colors.primary.darken(30%),
    // Titles
    ...
    progress-bar: new-base-colors.primary.lighten(30%),
